...
The workspace has 18 library crates, 3 application binaries (`apps/bench`, `apps/camera-face-tool`, `apps/llm-bench`), and an examples crate (26 examples in `examples/src/`). There are 1,861 default tests (1,897 with `--features "rknn metal-backend gpu realtime rknn-validate"`) across the 18 crates, criterion microbenchmarks, and CI with regression gates on GitHub Actions (macOS + Linux + Windows + ARM64). All crates share workspace version `0.1.10`.
