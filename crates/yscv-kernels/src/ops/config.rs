...
// Per-op-family parallel-dispatch thresholds. The rayon fork-join path has a
// fixed per-call cost; below these thresholds the dispatch overhead exceeds
// the compute savings. Env overrides (`YSCV_MIN_PAR_POINTWISE_CONV_ELEMS`,
// `YSCV_MIN_PAR_POINTWISE_CONV_FLOPS`, etc.) allow per-host tuning.

/// Minimum output elements AND FLOPs for pointwise (1×1) Conv to take the
/// row-parallel path. 16 384 elems ≈ 64 KB output; the FLOP gate ensures
/// per-thread work is large enough to amortise the pool wake-up.
