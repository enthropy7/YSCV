    ...
    #[cfg(all(target_arch = "arm", feature = "neon-v7"))]
    {
        ...
        if n > 0 {
            core::arch::asm!(
                ...
                // Two k-steps per iteration, so the loop's own three
                // instructions are amortised over sixteen FMAs, and both
                // streams are prefetched — neither is a stride the hardware
                // prefetcher follows across microkernel calls.
                "subs {n}, {n}, #1",
                "beq 4f",
                "3:",
                "vld1.32 {{d16-d17}}, [{a}]!",
