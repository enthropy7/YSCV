...
pub fn winograd4x4_transform_weights_f16(weights_f32: &[f32], ic: usize, oc: usize) -> Vec<u16> {
    let k = 9 * ic;
    assert_eq!(weights_f32.len(), k * oc);
...
impl MetalInference {
    pub fn new() -> Option<Self> {
        ...
        Some(MetalInference {
            ...
            winograd_batched_gemm_simd_f16io: pipe(&wino_lib, "winograd_batched_gemm_simd_f16io")?,
            winograd_output_transform_f16: pipe(&wino_lib, "winograd_output_transform_f16")?,
            // Winograd F(4×4, 3×3) conv
            winograd4x4_input_transform_f16: pipe(&wino_lib, "winograd4x4_input_transform_f16")?,
            winograd4x4_batched_gemm_f16io: pipe(&wino_lib, "winograd4x4_batched_gemm_f16io")?,
            winograd4x4_output_transform_f16: pipe(&wino_lib, "winograd4x4_output_transform_f16")?,
    ...
    pub fn buffer_from_f32(&self, data: &[f32]) -> Buffer {
        // SAFETY: (category 1) f32 slice reinterpreted as byte slice; len * 4 bytes.
        let bytes =
    ...
    pub fn buffer_from_f32_as_f16(&self, data: &[f32]) -> Buffer {
        ...
        // SAFETY: (category 1) u16 slice reinterpreted as byte slice; len * 2 bytes.
        let bytes = unsafe {
    ...
    pub fn read_buffer_f32(&self, buf: &Buffer, count: usize) -> Vec<f32> {
        ...
        // SAFETY: (category 1) buffer contents pointer valid; count checked above.
    ...
    pub fn write_buffer_f32(&self, buf: &Buffer, data: &[f32]) {
        ...
        // SAFETY: (category 2) destination length checked via debug_assert; no overlap.
        unsafe {
    ...
    pub fn write_buffer_f32_nchw_as_f16_nhwc(
        ...
        if c == 3 {
            for b_idx in 0..batch {
                ...
                for chunk in 0..chunks {
                    let s = chunk * 4;
                    // SAFETY: (category 3) aarch64 cfg guard; s + 4 <= spatial; dst offset within n * 2 bytes.
                    unsafe {
                ...
                for s in (chunks * 4)..spatial {
                    // SAFETY: (category 1) s < spatial; dst offset within buffer bounds.
                    unsafe {
                        let d = dst.add(dst_base + s * 3);
        ...
        for b_idx in 0..batch {
            ...
            for ch in 0..c {
                ...
                for chunk in 0..chunks {
                    let s = chunk * 4;
                    // SAFETY: (category 3) aarch64 cfg guard; s + 4 <= spatial; scatter writes within bounds.
                    unsafe {
                        let mut h4 = [0u16; 4];
                        #[cfg(target_arch = "aarch64")]
                        {
                ...
                for s in (chunks * 4)..spatial {
                    // SAFETY: (category 1) s < spatial; scatter write within buffer.
                    unsafe {
    ...
    pub fn read_buffer_f16_as_f32(&self, buf: &Buffer, count: usize) -> Vec<f32> {
        ...
        // SAFETY: (category 1) buffer contents pointer valid; count * 2 <= buf.length() checked above.
...
pub fn mps_gemm_f16(
    ...
    // SAFETY: (category 4) ObjC classes checked via Class::get; objects released after use;
    ...
    autoreleasepool(|| unsafe {
        ...
        let mps_mm_cls = Class::get("MPSMatrixMultiplication").ok_or_else(|| KernelError::Gpu {
...
pub fn mps_gemm_f16_encode(
    ...
    // SAFETY: (category 4) ObjC classes checked via Class::get; objects released after use;
    // autoreleasepool drains factory objects.
    autoreleasepool(|| unsafe {
        let mps_float16: u32 = 0x10000000 | 16;
        let mps_matrix_desc_cls =
            Class::get("MPSMatrixDescriptor").ok_or_else(|| KernelError::Gpu {
...
impl<'a> MetalEncoder<'a> {
    ...
    pub fn permute_0213(&self, input: &Buffer, out: &Buffer, d0: u32, d1: u32, d2: u32, d3: u32) {
        #[repr(C)]
        struct P {
            d0: u32,
            d1: u32,
...
impl MetalConv {
    ...
    pub fn read_buffer_f32(&self, buf: &Buffer, count: usize) -> Vec<f32> {
        let ptr = buf.contents() as *const f32;
        // SAFETY: (category 1) buffer contents pointer valid; caller ensures count fits.
        unsafe { std::slice::from_raw_parts(ptr, count).to_vec() }
    }
}
