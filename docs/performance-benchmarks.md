...
yscv runs NHWC. See [`docs/onnx-cpu-kernels.md`](onnx-cpu-kernels.md) for the
per-op hot-path map.
...
are generated by [`run-single-compute.sh`](../benchmarks/run-single-compute.sh)
and recorded in
[`single-compute-zen4-2026-06-09.md`](../benchmarks/single-compute-zen4-2026-06-09.md)
(commit `241f36c`, PyTorch 2.12.0, ONNX Runtime 1.26.0). The NumPy column is
produced separately by
[`bench_numpy_single_ops.py`](../benchmarks/python/bench_numpy_single_ops.py)
...
- *Memory-bound elementwise (add, mul, broadcast):* yscv is at **parity** with
  ...
  marginally *slower* than PyTorch on `add` (100 vs 98 µs) and the broadcast
  variants (72 vs 67, 69 vs 68 µs). It is faster than ORT on all of them
  (1.1–1.4×).
- *Transcendentals / activations:* yscv's polynomial-approximation kernels win
  substantially over PyTorch — `exp` ~5.8×, `tanh` ~10.7×, `gelu` ~19×,
  `sigmoid` ~4.9×, `silu` ~2.2× — and beat NumPy by similar or larger margins
  (`gelu` ~25×, `silu` ~19×). These are approximations to within a float
  tolerance, not bit-exact transcendental functions; that is the tradeoff that
  buys the speed.
- *Reductions / normalization:* yscv wins on `sum`, `max`, `softmax`,
  `layer_norm`, and `batch_norm` against all three.
- *vs ORT-CPU overall:* yscv is ahead on every op in the table, by roughly
  1.1–2.9× (and 10× on `layer_norm`, where ORT is unexpectedly slow at this
  shape).
...
The yscv/PyTorch/ORT snapshot records git commit and dirty state, toolchain
...
> Current Zen 4 single-thread activation numbers are in the
> [single-operation compute](#single-operation-compute-zen-4-1-thread) section
> above; this macOS table is retained for reference only.
...
> Historical optimization arc, retained for context. The **current** Zen 4
> tracker figures are in the
> [Siamese tracker — CPU inference](#siamese-tracker--cpu-inference-vs-onnx-runtime)
> section at the top of this document (min-of-300, commit `241f36c`). The p50
> numbers below predate that re-measurement and use a different run protocol.
